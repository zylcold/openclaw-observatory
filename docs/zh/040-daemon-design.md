...
- 后端查询/指标：默认 `127.0.0.1:10087`。
- 独立 Web/静态代理：默认 `127.0.0.1:10086`。

守护进程不再嵌入前端资源。`observatory-web` 从磁盘读取带版本的 Vite 构建，使用 `no-cache` 提供 `index.html`，将带哈希的静态资源作为 immutable 内容提供，并把 REST、SSE、健康检查和指标代理到后端。因此仅发布前端时无需重启守护进程。
...
公开的 `/health` 和 `/ready` 由 Web 服务代理。`/health` 表示后端进程可以响应。`/ready` 要求可写的数据库、有界的摄入 socket 和已完成的迁移。Gateway 宕机是运行状态，不是守护进程未就绪状态。
