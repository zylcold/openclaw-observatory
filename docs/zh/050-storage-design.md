...
| `sessions` | `(instance_id, session_id)` PK；启动/结束/状态/原因/消息数；FK instance | start, status |
| `agent_runs` | `(instance_id, run_id)` PK；`session_id` FK；提供商/模型/状态/时间/持续时间 | session, start, status |
