...
export function shell({ config, data, filters, loading, error, settingsOpen, sessionDetail, connectionLost, dataStale, kpiEditorOpen, customBuilder }) {
  ...
  const coreModules = visible.filter((m) => m.id === "overview");
  const detailModules = visible.filter((m) => m.id !== "overview");
  ...
  return `
    ...
    <main>
      <div class="page-title"><div><h1>运行概览</h1><p>${filters.range.toUpperCase()}　${filters.instanceId ? '实例 ' + esc(filters.instanceId) + ' · ' : ''}${filters.agentId ? 'Agent ' + esc(filters.agentId) + ' · ' : ''}${fmtLocal(filters.from)} — ${fmtLocal(filters.to)}</p></div><div class="page-actions"><button id="custom-chart-create" class="create-chart-button"><span>＋</span>创建图表</button><button id="refresh">${loading ? "刷新中…" : "立即刷新"}</button></div></div>
      ${error ? `<div class="banner error">${esc(error)}</div>` : ""}
      <section id="dashboard" class="dashboard ${loading && !data ? "loading" : ""}">${data ? coreModules.map((m) => moduleHTML(m.id, data, config, sessionDetail, kpiEditorOpen)).join("") + config.customCharts.map((item) => customChartPanelHTML(item, data)).join("") + detailModules.map((m) => moduleHTML(m.id, data, config, sessionDetail, kpiEditorOpen)).join("") : `<div class="skeleton">正在加载 v0.4 指标…</div>`}</section>
    </main>
    <div class="drawer-backdrop ${settingsOpen ? "open" : ""}" id="drawer-backdrop"></div>
    <aside class="drawer ${settingsOpen ? "open" : ""}">
      <header><div><h2>Dashboard 配置</h2><p>核心模块、刷新、自定义图表与阈值均保存为 JSON</p></div><button id="settings-close">×</button></header>
    ...
    ${customChartBuilderHTML(customBuilder)}`;
}
