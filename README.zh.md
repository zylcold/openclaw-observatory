...
| 守护进程 | `cmd/observatoryd`, `internal/` | 接收、校验、去重、归约、持久化、采样和服务 |
| 事件契约 | `schemas/` | Draft 2020-12 信封和载荷限制 |
| 本地 Dashboard | `web/` | 守护进程提供的轻量状态和近期事件 UI |
...
- `observatory.pid` — 安装脚本管理时的守护进程 PID 文件。

运行 `scripts/install-local.sh` 可创建每用户的 macOS 后台服务。它会构建守护进程、安装 LaunchAgent、链接插件、启用插件并重启 Gateway。使用 `scripts/uninstall-local.sh` 可移除服务；除非明确删除，数据库会被保留。
...
Docker Desktop 会抓取 `host.docker.internal:10086`。守护进程默认仅绑定 `127.0.0.1`，因此容器抓取是显式开启的：仅在受信任的宿主防火墙后使用 `-listen 0.0.0.0:10086` 启动，或直接在宿主上运行 Prometheus。REST API 包含本地运维标识符，不得暴露到不受信任的网络。
